...
void ShadowSystem::RenderDirectionalShadows(const SoftDirectionalLight& light,
    ...
    // Rasterize each cascade in parallel — each cascade has its own depth buffer
    int cascadeCount = static_cast<int>(m_cascades.size());
    ThreadPool::Get().ParallelFor(0, cascadeCount, [&](int c) {
