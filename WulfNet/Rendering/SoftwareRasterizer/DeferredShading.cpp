...
static Vec3 ClampVec(const Vec3& v) {
...
void DeferredShading::Apply(GBuffer& gbuffer, const DeferredShadingConfig& config,
    ...
    for (int y = 0; y < height; ++y) {
        for (int x = 0; x < width; ++x) {
            ...
            // Compute lighting
            Vec3 lit = ComputeLighting(normal, viewDir, depth, albedo, config, worldPos);
...
Vec3 DeferredShading::ComputeLighting(const Vec3& worldNormal, const Vec3& viewDir,
    ...
    Vec3 ambient = Vec3::Lerp(config.ambientGroundColor, config.ambientSkyColor, upFactor)
                       * config.ambientIntensity;
    color = color + Vec3{albedo.x * ambient.x, albedo.y * ambient.y, albedo.z * ambient.z};

    // --- Directional (sun) light ---
    Vec3 lightDir = config.sunLight.direction.Normalized() * -1.0f;
    ...
    color = Vec3::Lerp(color, config.ambientSkyColor, fresnel * 0.3f);

    // --- Distance fog ---
    float fogFactor = Clamp01((depth - config.fogStart) / (config.fogEnd - config.fogStart));
    color = Vec3::Lerp(color, config.fogColor, fogFactor);
