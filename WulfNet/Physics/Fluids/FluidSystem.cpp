...
// Diagnostic category for all FluidSystem log output
static constexpr const char* LOG_CAT = "Fluid";
...
FluidSystem::FluidSystem(const FluidSystemConfig& config, JPH::PhysicsSystem* physicsSystem)
...
void FluidSystem::AddWater(uint32_t x, uint32_t y, float vol) {
...
void FluidSystem::RemoveWater(uint32_t x, uint32_t y, float vol) {
    if (x >= m_config.width || y >= m_config.height) {
...
// -------------------------------------------------------------
void FluidSystem::StepSimulationCPU(float dt) {
    WULFNET_ZONE_NAMED("Fluid::StepCPU");
    WULFNET_SCOPED_TIMER("Fluid::StepCPU");
...
void FluidSystem::BuildSparseActiveTilesCPU() {
    WULFNET_ZONE_NAMED("Fluid::BuildSparse");
    m_activeTiles.clear();
...
void FluidSystem::InitializeGPUBuffers() {
    WULFNET_ZONE();
#ifdef WULFNET_HAS_VULKAN
    if (m_gpuInitialized) return;

    m_gpuCompute = std::make_unique<SWEComputeGPU>();
    if (!m_gpuCompute->Initialize(m_config.width, m_config.height)) {
        WULFNET_WARNING(LOG_CAT, "GPU SWE init failed — falling back to CPU");
        m_gpuCompute.reset();
        return;
    }

    // Upload the current CPU grid state to GPU.
    // SWEComputeGPU stores vec4 per cell: (waterHeight, terrainHeight, vx, vz).
    // We pack our SoA state into the interleaved layout.
    uint32_t totalCells = m_config.width * m_config.height;
    std::vector<float> packed(totalCells * 4);
    for (uint32_t i = 0; i < totalCells; ++i) {
        float waterH = m_state.terrainHeight[i] + m_state.waterDepth[i];
        packed[i * 4 + 0] = waterH;                  // waterHeight (absolute)
        packed[i * 4 + 1] = m_state.terrainHeight[i]; // terrainHeight
        packed[i * 4 + 2] = 0.0f;                     // vx (flux-based, start at 0)
        packed[i * 4 + 3] = 0.0f;                     // vz
    }

    if (!m_gpuCompute->UploadGrid(packed.data(), totalCells)) {
        WULFNET_WARNING(LOG_CAT, "GPU SWE upload failed — falling back to CPU");
        m_gpuCompute.reset();
        return;
    }

    m_gpuInitialized = true;
    WULFNET_INFO(LOG_CAT, "GPU SWE compute initialized (" +
                 std::to_string(m_config.width) + "x" + std::to_string(m_config.height) + ")");
#else
    WULFNET_INFO(LOG_CAT, "InitializeGPUBuffers: Vulkan not available, using CPU path");
#endif
}

void FluidSystem::RequestAsyncReadback() {
    WULFNET_ZONE();
#ifdef WULFNET_HAS_VULKAN
    if (!m_gpuInitialized || !m_gpuCompute) {
        WULFNET_TRACE(LOG_CAT, "RequestAsyncReadback: GPU not initialized");
        return;
    }

    if (!m_gpuCompute->DownloadGridAsync()) {
        WULFNET_WARNING(LOG_CAT, "RequestAsyncReadback: async download failed or already pending");
    }
#endif
}

void FluidSystem::DispatchCompute(float deltaTime) {
    WULFNET_ZONE();
#ifdef WULFNET_HAS_VULKAN
    if (!m_gpuInitialized || !m_gpuCompute) {
        WULFNET_TRACE(LOG_CAT, "DispatchCompute: GPU not initialized, skipping");
        return;
    }

    SWESimParams params{};
    params.gridSizeX = m_config.width;
    params.gridSizeZ = m_config.height;
    params.gravity_over_cs = m_config.gravity / m_config.gridSize;
    params.damping = m_config.fluxDamping;
    params.viscosity = 0.0f;  // No viscosity in base SWE
    params.dt = deltaTime;

    m_gpuCompute->StepSWE(params);

    // Check if a previous async readback has completed; if so, update CPU state
    if (m_gpuCompute->IsReadbackReady()) {
        std::vector<float> packed;
        if (m_gpuCompute->GetReadbackData(packed)) {
            uint32_t totalCells = m_config.width * m_config.height;
            // Unpack vec4 (waterHeight, terrainHeight, vx, vz) back to SoA
            for (uint32_t i = 0; i < totalCells && (i * 4 + 3) < packed.size(); ++i) {
                float waterHeight = packed[i * 4 + 0];
                float terrainH   = packed[i * 4 + 1];
                m_state.terrainHeight[i] = terrainH;
                m_state.waterDepth[i] = std::max(0.0f, waterHeight - terrainH);
            }
            WULFNET_TRACE(LOG_CAT, "GPU readback applied to CPU state");
        }
    }
#else
    WULFNET_TRACE(LOG_CAT, "DispatchCompute: no GPU backend, dt=" + std::to_string(deltaTime));
#endif
}

float FluidSystem::SampleWaterSurfaceHeight(float worldX, float worldZ) const {
...
void FluidSystem::ApplyBuoyancyForces(JPH::JobSystem* jobSystem) {
    WULFNET_ZONE_NAMED("Fluid::Buoyancy");
    if (!m_joltSystem) {
    ...
    for (int startIndex = 0; startIndex < numBodies; startIndex += batchSize) {
        ...
        jobHandles.emplace_back(jobSystem->CreateJob(
            "FluidBuoyancyBatch", JPH::Color::sCyan,
