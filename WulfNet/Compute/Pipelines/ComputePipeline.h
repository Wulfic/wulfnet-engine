...
public:
    ...
    /// Record a dispatch into an externally-managed command buffer (for async/batched use).
    /// The caller is responsible for beginning/ending/submitting the command buffer.
    /// Use with VulkanContext::BeginFrame() / SubmitFrame() for frame-pipelined dispatch.
    void DispatchAsync(void* cmdBuffer, uint32_t groupCountX,
                       uint32_t groupCountY = 1, uint32_t groupCountZ = 1);

    /// Record a compute→compute memory barrier into a command buffer.
    /// Used between DispatchAsync calls when the next dispatch reads from the previous write.
    static void RecordComputeBarrier(void* cmdBuffer);
