...
void ComputePipeline::DispatchAsync(void* cmdBuffer, uint32_t groupCountX,
                                     uint32_t groupCountY, uint32_t groupCountZ) {
    WULFNET_ZONE();

    if (!IsValid() || !cmdBuffer) {
        WULFNET_ERROR("Compute", "Cannot dispatch: invalid pipeline or null command buffer");
        return;
    }
    ...
    VkCommandBuffer cb = static_cast<VkCommandBuffer>(cmdBuffer);

    // Bind pipeline
    s_vkCmdBindPipeline(cb, VK_PIPELINE_BIND_POINT_COMPUTE, m_pipeline);

    // Bind descriptor set
    s_vkCmdBindDescriptorSets(cb, VK_PIPELINE_BIND_POINT_COMPUTE,
                              m_pipelineLayout, 0, 1, &m_descriptorSet, 0, nullptr);

    // Push constants
    if (m_pushConstantSize > 0) {
        s_vkCmdPushConstants(cb, m_pipelineLayout,
                            VK_SHADER_STAGE_COMPUTE_BIT, 0,
                            m_pushConstantSize, m_pushConstantData.data());
    }

    // Dispatch — no submit, caller manages the command buffer lifecycle
    s_vkCmdDispatch(cb, groupCountX, groupCountY, groupCountZ);
}

void ComputePipeline::RecordComputeBarrier(void* cmdBuffer) {
    if (!cmdBuffer || !s_vkCmdPipelineBarrier) return;
